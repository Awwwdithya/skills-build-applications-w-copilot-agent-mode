...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        Activity.objects.create(user=captain, type='Swim', duration=45, date=timezone.now().date())
        Activity.objects.create(user=batman, type='Cycle', duration=60, date=timezone.now().date())
        Activity.objects.create(user=superman, type='Fly', duration=120, date=timezone.now().date())
